...
export function getStrategyAdjustmentDefaults(option) {
  const quantityAction = option?.actions?.find((action) => action.actionQuantity) ?? option?.actions?.[0];
  const discountAction = option?.actions?.find((action) => action.actionType === 'PRICE_DISCOUNT');

  return {
    quantity: quantityAction?.actionQuantity ?? 0,
    discountPercent: (discountAction?.discountRate ?? 0) * 100,
    startDate: quantityAction?.startDate ?? '',
    endDate: quantityAction?.endDate ?? '',
    strategyPrice: discountAction?.strategyPrice ?? quantityAction?.strategyPrice ?? 0,
    actionCost: option?.actions?.reduce((sum, action) => sum + (action.estimatedActionCost ?? 0), 0) ?? 0,
  };
}
...
export function buildAdjustedStrategyOption(strategyCase, option, adjustment) {
  ...
  const initialStock = strategyCase.baselineSimulation?.dailySeries?.[0]?.expectedRemainingQty ?? defaults.quantity;
  const quantity = clamp(adjustment.quantity, 0, initialStock);
  const discountPercent = clamp(adjustment.discountPercent, 0, 50);
  const strategyPrice = Math.max(Number(adjustment.strategyPrice) || 0, 0);
  const actionCost = Math.max(Number(adjustment.actionCost) || 0, 0);
  const baseSales = option.simulationSummary.expectedSalesQty || 1;
  const baseQuantity = defaults.quantity || 1;
  const baseDuration = getInclusiveDays(defaults.startDate, defaults.endDate);
  const adjustedDuration = getInclusiveDays(adjustment.startDate, adjustment.endDate);
  const discountLift = Math.max(0.5, 1 + ((discountPercent - defaults.discountPercent) / 100) * 1.5);
  ...
  const expectedSalesQty = Math.min(
    initialStock,
    quantity,
    Math.max(0, Math.round(baseSales * discountLift * durationLift * quantityLift)),
  );
  ...
  const adjustedUnitMargin = baseUnitMargin + (strategyPrice - defaults.strategyPrice);
  ...
  let remainingActionCost = actionCost;
  const actions = option.actions.map((action, index) => {
    const estimatedActionCost = index === 0 ? remainingActionCost : 0;
    remainingActionCost -= estimatedActionCost;
    return {
      ...action,
      actionQuantity: action.actionQuantity ? quantity : action.actionQuantity,
      discountRate: action.actionType === 'PRICE_DISCOUNT' ? discountPercent / 100 : action.discountRate,
      strategyPrice,
