...
describe('AI strategy selection conflict', () => {
  it('409 selection conflict만 전용 오류로 판별한다', () => {
    expect(isAiStrategySelectionConflict({ status: 409, code: 'AI_STRATEGY_SELECTION_CONFLICT' })).toBe(true);
    expect(isAiStrategySelectionConflict({ status: 409, code: 'OTHER_CONFLICT' })).toBe(false);
    expect(isAiStrategySelectionConflict({ status: 500, code: 'AI_STRATEGY_SELECTION_CONFLICT' })).toBe(false);
  });
