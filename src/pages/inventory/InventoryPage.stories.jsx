...
import { InventoryPageSkeleton } from './ui/InventoryPageSkeleton.jsx';
...
function createInventoryStoryClient() {
  return createStoryQueryClient((client) => {
    inventoryItems.forEach((item, index) => {
      item.salesPoints.forEach((salesPoint) => {
      ...
      // 상세 드로어는 미지정 재고 위험도도 함께 조회하므로 모든 SKU에 대한
      // 미지정 키를 미리 채워 Storybook에서 백엔드 요청이 발생하지 않게 합니다.
      client.setQueryData(
        riskQueryKeys.detail(item.skuCode, 'UNASSIGNED'),
        mapRiskAssessmentResponse({
          ...getMockRiskAssessmentDto(item.skuCode, 'UNASSIGNED'),
          availableQty: item.unassignedInventory?.availableQuantity ?? 0,
          safetyStockQty: item.safetyQuantity,
        }),
      );
    });
