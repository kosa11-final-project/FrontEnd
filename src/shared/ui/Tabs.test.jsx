...
function TabsWithDisabledFixture() {
  const [value, setValue] = useState('overview');
    ...
    <Tabs value={value} onValueChange={setValue}>
      {({ value: activeValue, setValue: selectValue }) => (
        <TabsList aria-label="재고 상세 탭">
          ...
          <TabsTrigger value="forecast" activeValue={activeValue} onSelect={selectValue} disabled>
            수요예측
          </TabsTrigger>
          <TabsTrigger value="lots" activeValue={activeValue} onSelect={selectValue}>
            LOT
...
describe('Tabs', () => {
  ...
  it('skips disabled tabs during arrow-key navigation and selects the next enabled tab', () => {
    render(<TabsWithDisabledFixture />);
